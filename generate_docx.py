...
# ----- 2. The Problem -----
add_heading(doc, "2. הבעיה העסקית", level=1)
add_heading(doc, "מי הצרכן", level=2)
add_para(doc, "אנליסט מחקר ב-i24 — צריך לחזות רייטינג של תוכנית לפני שהיא משודרת, "
              "לתכנון לוח שידורים.")
add_heading(doc, "השאלה", level=2)
add_quote(doc, "מה הרייטינג הצפוי לקבינט שישי בשבוע הבא?")
add_quote(doc, "מה אם נעביר את חדר חדשות 21 לרביעי?")
add_heading(doc, "למה זה חשוב", level=2)
add_bullets(doc, [
    "כל סטיית רייטינג של 0.1 = השפעה מסחרית (מחיר פרסום, חוזים)",
    "מנהלי תוכן בונים לוח שידורים עד חודש מראש",
    "כיום: ההחלטות מבוססות על ניחוש מומחה + אקסל",
])
